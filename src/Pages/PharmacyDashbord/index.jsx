...
export default function PharmacyDashboard() {
    ...
    const seedInventory = [
        { id: 'INV-001', name: 'Paracetamol 500mg', manufacturer: 'ACME Pharma', price: '4.99', quantity: '120', description: 'Pain reliever' },
        { id: 'INV-002', name: 'Amoxicillin 250mg', manufacturer: 'HealthCorp', price: '7.50', quantity: '60', description: 'Antibiotic' },
        { id: 'INV-003', name: 'Loratadine 10mg', manufacturer: 'GoodMeds', price: '6.25', quantity: '80', description: 'Allergy relief' },
    ];
                            ...
                            <div className="lg:col-span-1">
                                <div className="bg-white p-6 rounded-lg shadow-sm">
                                    ...
                                    <form onSubmit={handleAddMedicine} className="space-y-3">
                                        <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Medicine Name" className="w-full px-4 py-2 border rounded" />
                                        <input value={form.manufacturer} onChange={(e) => setForm({ ...form, manufacturer: e.target.value })} placeholder="Manufacturer" className="w-full px-4 py-2 border rounded" />
                                        <div className="grid grid-cols-2 gap-3">
                                            <input value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} placeholder="Price" className="px-4 py-2 border rounded" />
                                            <input value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })} placeholder="Quantity" className="px-4 py-2 border rounded" />
                                        </div>
                                        <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Description (optional)" className="w-full px-4 py-2 border rounded" />
                                        <div className="flex gap-3">
                                            <button type="submit" className="bg-[#006d77] text-white px-4 py-2 rounded">Add</button>
                                            <button type="button" onClick={() => setForm({ name: '', manufacturer: '', price: '', quantity: '', description: '' })} className="bg-gray-100 px-4 py-2 rounded">Reset</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                    ...
                    {view === 'inventory' && (
                        <div className="bg-white p-6 rounded-lg shadow-sm">
                            <div className="flex justify-between items-center mb-4">
                                ...
                                <button onClick={() => setView('dashboard')} className="bg-[#006d77] text-white px-4 py-2 rounded">+ Add Medicine</button>
                            </div>
                            <div className="overflow-x-auto">
                                <table className="min-w-full">
                        ...
                        <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-md p-10">
                            <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
                                <div className="flex flex-col items-center">
                                    <img src="https://placehold.co/96x96/E0E7FF/4F46E5?text=RX" alt="Pharmacy" className="w-40 h-40 rounded-full border-4 border-[#006d77]" />
                                    <button className="mt-4 px-4 py-2 bg-[#006d77] text-white rounded-lg hover:bg-[#005a63] transition">Change Photo</button>
                                </div>
                                <div className="flex-1 w-full">
                                    <h2 className="text-2xl font-semibold text-[#006d77] mb-6">Pharmacy Information</h2>
                                    <form className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div>
                                            <label className="block text-sm font-medium text-gray-600 mb-2">Pharmacy Name</label>
                                            <input type="text" defaultValue="CureLink Pharmacy" className="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-[#006d77]" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-600 mb-2">Email</label>
                                            <input type="email" defaultValue="pharmacy@curelink.com" className="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-[#006d77]" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-600 mb-2">Phone</label>
                                            <input type="text" defaultValue="(02) 12345678" className="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-[#006d77]" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-600 mb-2">Branch</label>
                                            <input type="text" defaultValue="Main Branch" className="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-[#006d77]" />
                                        </div>
                                        <div className="md:col-span-2">
                                            <label className="block text-sm font-medium text-gray-600 mb-2">Address</label>
                                            <input type="text" defaultValue="Giza, Egypt" className="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-[#006d77]" />
                                        </div>
                                        <div className="md:col-span-2">
                                            <label className="block text-sm font-medium text-gray-600 mb-2">Working Hours</label>
                                            <input type="text" defaultValue="Mon-Fri 9:00 AM - 9:00 PM" className="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-[#006d77]" />
                                        </div>
                                    </form>
                                    <div className="flex justify-end mt-8">
                                        <button className="px-6 py-2 rounded-lg bg-[#006d77] text-white hover:bg-[#005a63] transition">Save Changes</button>
                                    </div>
                                </div>
                            </div>
                        </div>

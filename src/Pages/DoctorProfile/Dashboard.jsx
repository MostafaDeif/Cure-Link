    ...
    <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100 flex flex-col gap-4 hover:shadow-xl transition-all duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {appointments.map((appt, index) => (
          <div
            key={index}
            className="flex justify-between items-center p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-all duration-300"
          >
            <div>
            ...
            <span
              className={`px-4 py-1 rounded-full font-semibold text-sm ${getStatusClass(appt.status)}`}
            >
              {appt.status}
            </span>
          </div>
  ...
  return (
    <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100 flex flex-col gap-4 hover:shadow-xl transition-all duration-300">
      ...
      <button
        ...
        className="w-full py-3 rounded-full bg-[#006d77] text-white font-semibold hover:bg-[#004f52] transition-all duration-300"
      >
        + Add Appointment
      </button>
      <button
        onClick={() => navigate("/doctor-patients")}
        className="w-full py-3 rounded-full border border-gray-300 bg-white text-gray-700 font-semibold hover:bg-gray-50 transition-all duration-300"
      >
    ...
    <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300">
      ...
      <ul className="flex flex-col gap-3">
        {patients.map((patient, idx) => (
          <li
            key={idx}
            className="flex justify-between items-center p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-all duration-300"
          >
...
const Dashboard = () => {
  ...
  return (
    <div className="flex min-h-screen bg-gray-100 transition-all">
      ...
      />
      <div
        className={`flex-1 flex flex-col overflow-hidden transition-all duration-500 ${sidebarOpen ? "ml-64" : "ml-0"}`}
      >
        <HeaderDoctor

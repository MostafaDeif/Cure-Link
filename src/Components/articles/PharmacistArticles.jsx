...
export default function PharmacistArticles() {
  const drugApprovals = [
    ...
    {
      id: 2,
      title: "دواء لعلاج الزهايمر المبكر",
      authority: "EMA 2025",
      status: "pending",
  ...
  const upcomingConferences = [
    {
      ...
      image:
        "https://i.pinimg.com/736x/2c/8a/90/2c8a9004feae986bbc7282ba4aa8cda2.jpg",
    ...
    },
    {
      ...
      image:
        "https://i.pinimg.com/1200x/31/8e/a1/318ea140097c3e7cd19ed6ba893ed7cb.jpg",
    ...
    <div className="space-y-10" dir="rtl">
      <section className="rounded-[32px] border border-[#e5f8f4] bg-gradient-to-br from-[#f6fffd] via-[#ecfbf7] to-[#def6f1] p-6 text-[#045453] shadow-[0_20px_50px_rgba(11,94,91,0.15)] md:p-8">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm uppercase tracking-[0.4em] text-[#3d8a85]">
              المحدث الآن
            </p>
            <h2 className="text-3xl font-bold text-[#045453]">
              آخر الموافقات الدوائية
            </h2>
          </div>
        </div>
        <div className="space-y-4">
          {drugApprovals.map((drug) => (
            <div
              key={drug.id}
              className="flex flex-wrap items-center justify-between gap-4 rounded-3xl border border-[#d7f5ef] bg-white px-5 py-4 text-right text-[#024443] shadow"
            >
              <div>
                <h3 className="text-lg font-semibold text-[#012f2f]">
                  {drug.title}
                </h3>
                <p className="text-sm text-[#0a6461]">{drug.authority}</p>
              </div>
              <span
                className={`rounded-full px-4 py-1 text-sm font-semibold ${statusStyles[drug.status] || "bg-slate-200 text-slate-700"}`}
              >
                {drug.statusText}
              </span>
            </div>
      ...
      <section className="space-y-6">
        <h2 className="text-3xl font-bold text-[#034543]">
          المؤتمرات الصيدلانية القادمة
        </h2>
        <div className="grid gap-6 md:grid-cols-2">
          {upcomingConferences.map((conference) => (
            <div
              key={conference.id}
              className="flex flex-col overflow-hidden rounded-[26px] border border-[#d2f6f0] bg-white shadow-[0_12px_30px_rgba(9,132,117,0.12)] transition hover:-translate-y-1.5"
            >
              ...
              <div className="flex flex-1 flex-col gap-4 p-5 text-right text-[#024443]">
                <div>
                  <h3 className="text-xl font-semibold text-[#012f2f]">
                    {conference.title}
                  </h3>
                  <p className="mt-1 text-sm text-[#0a6461]">
      ...
      <section className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {features.map((feature) => (
          <div
            key={feature.title}
            className="flex flex-col rounded-[28px] border border-[#d2f6f0] bg-white p-6 text-right shadow-[0_12px_30px_rgba(9,132,117,0.1)]"
          >
            <div className="mb-4 inline-flex rounded-2xl bg-[#e3fbf8] p-3">
              {feature.icon}
            </div>
            <h3 className="text-xl font-semibold text-[#023f3e]">
              {feature.title}
            </h3>
